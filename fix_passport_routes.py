...
def fix_passport_routes():
    ...
    try:
        ...
        content = re.sub(
            r"logger\.error\(f\"Error getting user passport: \{str\(e\)\}\"\)",
            r"logger.error(f\"Error getting user passport: {str(e)}\", exc_info=True)",
            content
        )
        
        content = re.sub(
            r"logger\.error\(f\"Error syncing user passport: \{str\(e\)\}\"\)",
            r"logger.error(f\"Error syncing user passport: {str(e)}\", exc_info=True)",
            content
        )
...
"""
            ...
            app_content = re.sub(
                r"(app\.register_blueprint\(passport_bp.*?\)\n)",
                r"\1\n" + test_endpoint,
                app_content
            )
