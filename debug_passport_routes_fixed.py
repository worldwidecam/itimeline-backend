...
def debug_passport_routes():
    ...
    if "from routes.passport import passport_bp" in app_content:
        print("[OK] Passport blueprint is imported in app.py")
    else:
        print("[FAIL] Passport blueprint is NOT imported in app.py")
    
    if "app.register_blueprint(passport_bp" in app_content:
        print("[OK] Passport blueprint is registered in app.py")
    else:
        print("[FAIL] Passport blueprint is NOT registered in app.py")
    ...
    try:
        ...
        if "import sqlite3" in passport_content:
            print("[OK] sqlite3 module is imported in passport.py")
        else:
            print("[FAIL] sqlite3 module is NOT imported in passport.py")
    except Exception as e:
        print(f"[FAIL] Error reading passport.py: {str(e)}")
    ...
    try:
        ...
        if os.path.exists(instance_db_path):
            ...
            if cursor.fetchone():
                print(f"[OK] user_passport table exists in {instance_db_path}")
                ...
                print(f"[FAIL] user_passport table does NOT exist in {instance_db_path}")
            conn.close()
        else:
            print(f"[FAIL] Database file {instance_db_path} does not exist")
        ...
        if os.path.exists(root_db_path):
            ...
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='user_passport'")
            if cursor.fetchone():
                print(f"[OK] user_passport table exists in {root_db_path}")
                ...
                print(f"[FAIL] user_passport table does NOT exist in {root_db_path}")
            conn.close()
        else:
            print(f"[FAIL] Database file {root_db_path} does not exist")
    except Exception as e:
        print(f"[FAIL] Error checking database: {str(e)}")
